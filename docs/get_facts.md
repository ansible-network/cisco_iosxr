...
The `get_facts` function can be used to collect facts from an Cisco iosxr-XR
...
The above playbook will return the facts for the host under the `cisco_iosxr`
top level key.  
...
- hosts: cisco_iosxr
  ...
  tasks:
    - name: collect facts from cisco iosxr devices
      import_role:
        name: ansible-network.cisco_iosxr
